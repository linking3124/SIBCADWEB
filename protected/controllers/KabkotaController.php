...
//			'postOnly + delete', // we only allow deletion via POST request
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','dataLokasi'),
				'users'=>array('@'),
			),
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Kabkota']))
		{
			...
			if($model->save())
				$this->redirect(array('view','id'=>$model->id_kabkota));
		}
	...
	{
		$dataProvider=new CActiveDataProvider('Kabkota');
	...
	public function actionDataLokasi()
	{
		...
		$this->layout='mainxml';
			...
			foreach($model as $db)
			{
				  ...
				  echo '<marker ';
				  echo 'id_kabkota="' . parseToXML($db->id_kabkota) . '" ';
				  echo 'nama="' . parseToXML($db->nama) . '" ';
				  echo 'lat="' . $db->lat . '" ';
				  echo 'lng="' . $db->lng . '" ';
				  
				  $modelTest = Kabkota::model()->getCount($db->id_kabkota);
		 		  foreach($modelTest as $test){
		 		  	 echo 'total_saksi="' . $test['total'] . '" ';
